...
#function to return whether user exists or not#
...
def ask():
    ...
    db = get_db()
    if request.method == 'POST':
        db.execute('insert into questions (questions_text, asked_by_id, expert_id) values (?,?,?)', [request.form['question'], user['id'],  request.form['expert'] ])
        db.commit()
        return redirect(url_for('index'))
